<script>
	...
	import Crossfilter from '$lib/crossfilter/Crossfilter.svelte';
	import Filter from '$lib/crossfilter/Filter.svelte';
	...
	async function tablechanged() {
		...
		if (tablespec) {
			// from file, rather than from server
		...
		allowedlayers = _.intersection(
			allowedlayers,
			layerlist.map((d) => d.db.field)
		);
	...
	function make_match(selection, dbfield, datefield, startDate, endDate) {
		...
		if (datefield && (startDate || endDate)) {
			newmatch[datefield] = {};
			if (startDate) {
				newmatch[datefield].$gte = { $date: dfunc.format(startDate, dateformat) };
			}
			if (endDate) {
				newmatch[datefield].$lt = { $date: dfunc.format(endDate, dateformat) };
			}
		}
	...
	function clean(data) {
		...
		data.map(function (d) {
			if (groups[d.cycle]) {
				groups[d.cycle].data.push(d);
			} else {
				groups[d.cycle] = { data: [d] };
			}
		});
		return groups;
	}
	...
	let data;
	function plot() {
		data = query('population', table.table, match).then(clean);
	}
...
<section class="container select-wrapper">
	<div class="row">
		<div class="col-md-6">
			<div class="box">
				...
				<div class="select control is-fullwidth">
					<Dropdown {whitelist} on:change={tablechanged} bind:table />
				</div>
			</div>
			<div class="box">
		...
		<div class="col-md-6">
			<div class="box">
				<Clear clearfn={cleardateselection} />
			...
			<div class="box">
				{#if data}
					...
					{:then groups}
						<WaterfallGraph {groups} bind:options />
					{/await}
				{/if}
			</div>
		</div>
...
<Crossfilter db="population" collection="noon_by_rto" active="true">
	<Filter
		id="startdate"
		bind:brush={startDate}
		pipeline={[{ name: 'startdate' }]}
		bind:data={startDateData}
	/>
	<Filter
		id="enddate"
		bind:brush={endDate}
		pipeline={[{ name: 'enddate' }]}
		bind:data={endDateData}
	/>
