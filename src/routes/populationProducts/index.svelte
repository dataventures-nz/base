								...
								{#await myextents then extent}
									<div class="flex">
										<DatePicker
											...
											isAllowed={(date) =>
												date <= endDate &&
												date >= mode.dateExtents(extent)[0] &&
												date <= mode.dateExtents(extent)[1]}
										/>
										<DatePicker
								...
								{#each layers as layer}
									<button
										style={'background-color:' +
											(layer.active ? layer.style.fill : 'white') +
											';color:black'}
						...
						{#if mode.population_before && gotyears}
							{#await gotyears then years}
								<select
									name="year1"
									id="year1"
									on:change={(e) => (yearIndex1 = e.target.options.selectedIndex)}
								>
									{#each years as yr, i}
						...
						{#if mode.population_before && gotyears}
							{#await gotyears then years}
								<select
									name="year2"
									id="year2"
									on:change={(e) => (yearIndex2 = e.target.options.selectedIndex)}
								>
									{#each years as yr, i}
