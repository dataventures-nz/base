<script>
  ...
  export let data=[{x:0,y1:0.3,y2:0.7},{x:1,y1:0.7,y2:0.3},{x:2,y1:0.5,y2:0.4}]
  ...
  $: if(yextent){yScale.setExtents(id,yextent)} 
    else if (internalYextent){yScale.setExtents(id,internalYextent)} 
    ...
    {#each data as point}
      {#each _layers as layer}
        <slot y={$yScale(layer.accessor(point))} x={$xScale(xaccessor(point))} >
          <circle r=5 cy={$yScale(layer.accessor(point))} cx={$xScale(xaccessor(point))} {...layer.style}></circle>
        </slot>
      {/each}
    {/each}
  </g>