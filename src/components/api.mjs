...
export const fetch_csv = async (method, url, body) => onServer?nothing : await csv(url, {method, headers: {Accept:"text/csv", ...await get_headers()}, body: body?bson.EJSON.stringify(body):undefined})
export const fetch_json = async(method, url, body) => onServer?nothing : await fetch(url, {method, headers: await get_headers(), body: body?bson.EJSON.stringify(body):undefined}).then(response => response.json())
