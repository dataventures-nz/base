<script>
  ...
  $: if (active && pipeline) { if (useMatches) { debounceStore.queue([...pre, ...other_matches, ...pipeline]) } else { debounceStore.queue([...pre, ...pipeline]) }}
  $: if (active && pipeline) { data = process($debounceStore) }
